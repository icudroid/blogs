...
public class CommentBlogItemResource {
    ...
    public CommentBlogItemResource(CommentBlogItemService commentBlogItemService, UserService userService) {
    ...
    public ResponseEntity<CommentBlogItemDTO> createCommentBlogItem(@Valid @RequestBody CommentBlogItemDTO commentBlogItemDTO) throws URISyntaxException {
        ...
        if (SecurityUtils.isCurrentUserInRole(AuthoritiesConstants.USER)) {
            ...
            commentBlogItemDTO.setAuthorId(user.get().getId());
        }

        commentBlogItemDTO.setCreated(ZonedDateTime.now());
        commentBlogItemDTO.setStatus(CommentBlogStatus.CREATED);
        commentBlogItemDTO.setTitle("title");
    ...
    @GetMapping("blog-items/{idBlogItem}/comments")
    @Timed
    public  ResponseEntity<List<CommentBlogItemWithAuthorDTO>> getCommentsByItem(@PathVariable Long idBlogItem, Pageable pageable) {
        log.debug("REST request to get CommentBlogItem : {}", idBlogItem);
        Page<CommentBlogItemWithAuthorDTO> page = commentBlogItemService.findByItem(idBlogItem,pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders("", page, "/api/blog-items/"+idBlogItem+"comment-blog-items");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
